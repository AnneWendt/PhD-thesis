...
The foveal area of the image is defined as the block with the most activity.
This focus block is assessed using colour vision.

How spikes are created in peripheral blocks:
When a pixel's brightness changes by more than a defined threshold between
frames, a spike is created for this pixel location. If there are more than a
defined number of spikes in a block, the block creates a spike.

How spikes are created in the focus/foveal block:
For each pixel, the BGR values are transformed into CIELAB colour space. Then
the CIEDE2000 formula is used to compute the delta E of the colours. If
delta E is larger than a defined threshold, this pixel creates a spike. If the
fovea pixels are to be summarised in blocks, a block creates a spike if there
are more than a defined number of spikes in a block.
...
# DIRECTORY2 = os.path.join('C:', os.sep, 'Users', 'em165153', 'Documents', 'JESTER', 'encoded')
DIRECTORY = 'randomly_selected_subset'
DIRECTORY2 = 'encoded_subset'
FILETYPE = '.mp4'  # only these files in the directory will be encoded

TEST_RUN = True  # play videos more slowly and do not close windows at the end

# these thresholds are used when comparing subsequent frames on a pixel base
# if they differ enough, a spike is created
PIXEL_THRESHOLD = 25  # applies to the outer blocks in greyscale
FOVEA_THRESHOLD = 5  # applies to the central block in colour = DELTA E JND

# how many blocks do we want in each row and column for each block level
# adjust for different template sizes
#          m  n
BLOCKS = [[4, 4],  # block 0 (outermost periphery)
#          [4, 4],  # block 1
          [4, 4],  # block 2
#          [4, 4],  # block 3
          [5, 5],  # block 4
...
# threshold for a periphery block to be counted as a spike
BLOCK_THRESHOLD = 0.1

# threshold for a fovea block to be counted as a spike
BLOCK_FOVEA_THRESHOLD = 0.5
...
def encode(filename):
    ...
    while success:
        ...
        # calculate the difference between pixel brightness
    ...
    # remove the first line that we filled with zeros when creating the sample array
...
def get_block_fovea(fovea_pixel_spikes, block_info):
    ...
    # keep track of current item in output spike row
    block_index = 0
    ...
    for col in range(BLOCKS[-1][0]):
        ...
        for row in range(BLOCKS[-1][1]):
            ...
            if block_mean > BLOCK_FOVEA_THRESHOLD:
                spikes[0, block_index] = 1

            # increase counter to keep track of next spike index
            block_index += 1
...
def get_block_info(frame_shape):
    ...
    # count number of pixels per block and total number of blocks
    block_widths = []
    block_heights = []
    ...
    # debug info
    if TEST_RUN:
        print("Number of input channels required for periphery: ",
              block_info['total_number'])
        print("Number of input channels required for fovea: ",
...
def get_block_spikes(pixel_spikes, block_info, boundaries, focus):
    """
    Calculates which block emits a spike. Also determines the most active region.
...
def get_fovea_spikes(frame_t0, frame_t1, fovea_boundaries):
    ...
    # set all elements > fovea_threshold to 1
...
def get_frame_diff_as_spikes(frame0, frame1):
    ...
    # set all elements > pixel_threshold to 1
...
def save_result_file(filename, sample):
    ...
    The file name is equal to the video name.
    """

    # save file as csv
    np.savetxt(os.path.join(DIRECTORY2, filename[:-4]) + '.csv',
               sample, fmt='%1.0f', delimiter=',')
